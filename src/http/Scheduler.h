...
class Scheduler {
  public:
    ...
    static std::chrono::system_clock::time_point get_midnight();
    static std::chrono::system_clock::time_point get_next_midnight();
    
    std::chrono::system_clock::time_point current_week_point() const;
    std::chrono::system_clock::time_point next_week_point() const;
    std::chrono::system_clock::time_point next_daily_point(int hour = 0, int munutes = 0, int seconds = 0) const;
    std::chrono::system_clock::time_point next_weekly_point(int target_wday, int hour = 0, int munutes = 0, int seconds = 0) const;
    static std::string time_point_to_string(const std::chrono::system_clock::time_point &tp);

    void schedule_after(std::chrono::system_clock::duration delay, std::function<void()> job);
    void schedule_at(std::chrono::system_clock::time_point timestamp, std::function<void()> job);
